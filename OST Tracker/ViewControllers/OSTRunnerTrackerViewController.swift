...
class OSTRunnerTrackerViewController: OSTBaseViewController, UITextFieldDelegate {
    ...
    override func viewWillDisappear(_ animated: Bool) {
        ...
        // Leaving the screen commits the held entry so it never stays stuck unsynced.
    ...
    private func makeBody() -> UIView {
        ...
        // tiny keys on a tall iPad portrait screen. Guarantee the pad a minimum
        // share of the body; the landscape side-by-side arrangement (full-height
        // pad) satisfies this trivially, so portrait is what grows. Below
        // required so the display zone's content still wins if space is tight.
        let padShare = padContainer.heightAnchor.constraint(
            greaterThanOrEqualTo: bodyStack.heightAnchor, multiplier: 0.30)
    ...
    @IBAction func onEntryButton(_ sender: Any) {
        ...
        lastEntry = entry
        // Hold this entry out of Auto Sync until the user confirms or moves on.
    ...
    /// Commit the held entry: release it to Auto Sync and reset the screen for the
    /// next bib. (Recording another runner or typing a new bib does this implicitly.)
    @objc func onConfirmEntry(_ sender: Any) {
        OSTSound.shared().play("click")
        AutoSyncController.shared.releaseHeldEntry()
        cleanData()
        lblRunnerInfo.isHidden = true
