...
- ⏭️ **NEXT: Task 3 — APIClient + Codable models.** Add `Verification/fixtures` to the test target as a **folder reference** (blue folder) so `Fixture.data(...)` can load JSON from the test bundle (use the `xcodeproj` gem; `new_reference` with the dir, set as resource on the test target). Verify Codable field names against the real JSON in `Verification/fixtures/*.json` before asserting. Module `OST_Remote`; add Swift files to targets via the `xcodeproj` gem (pattern in commits `84fba4c`/`8523d39`).
