...
- **Deferred (user will get to it later): the Edit/Create-Entry screen** (`OSTEditEntryViewController`, still Obj-C; shared by Review/Sync + tracker). Pre-existing issues on tall devices: it sets IQKeyboardManager `enableAutoToolbar = YES` (a stray Done bar floats over the APNumberPad) and has no safe-area layout for the Delete/Update buttons. Migrate it to Swift + apply the safe-area/toolbar fixes as its own task. (The one-line `enableAutoToolbar = NO` fix was reverted per the user, to keep it untouched for that pass.)
