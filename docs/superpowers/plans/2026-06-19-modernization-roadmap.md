...
## Phase 4 — Networking: AFNetworking → `APIClient` (started)
...
- **Remaining endpoints to migrate:** `getAllEvents`, `getEventsDetails`, `fetchNotExpected`, `submitEventGroupEntries` (and `OSTSyncManager`'s submit). The token is shared via `addTokenToHeader`; migrate incrementally, then delete `OSTNetworkManager` + drop AFNetworking.
