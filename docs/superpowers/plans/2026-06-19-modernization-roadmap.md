...
- **Phase 2.5 — Edit/Create-Entry screen → Swift, done, pending verify** (`OSTEditEntryViewController`, commit `3637b3d`; shared by Review/Sync + tracker). Edit + create-new flows ported (bib lookup, time/date pickers, pacer/stopped, update, delete). **Fixed the "weird" toolbar:** `enableAutoToolbar` YES→NO removes the stray Done bar over the number pad. Dropped the dead iPhone-X/XR nudge; `lblWithPacer` (unconnected outlet) made optional; OHAlertView→UIAlertController. _Watch on verify:_ if the Delete/Update buttons still sit with an awkward gap above the keypad, pin them to the bottom safe area as a follow-up.
